...
export const AdminModal: React.FC<AdminModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/85 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 animate-fadeIn">
      <div className="relative w-full max-w-2xl bg-[#0e1628] border border-amber-500/40 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        
        {/* Header */}
        <div className="p-4 sm:p-5 border-b border-slate-800 bg-[#0a101d] flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-lg bg-amber-950/80 border border-amber-700/60 text-amber-400 relative">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                {isAdmin
                  ? 'Review user flags, inspect reported items, and manage platform safety'
                  : 'Authorized administrators & moderators only'}
              </p>
            </div>
          </div>
          <button
        ...
        {!isAdmin ? (
          <div className="p-5 space-y-4">
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                {error && (
                  <p className="text-xs text-rose-400 flex items-center gap-1 mt-1.5 font-medium">
                    ...
                    Access key rejected. Please verify and try again.
                  </p>
                )}
              </div>
              ...
              <div className="text-[11px] text-slate-400 leading-relaxed bg-slate-900/60 p-3 rounded-lg border border-slate-800 space-y-1">
                <p>
                  Unlocking moderator console allows real-time inspection of flagged content, reports triage, and global deletion permissions.
                </p>
                {pendingReportsCount > 0 && (
                  <p className="text-rose-400 font-semibold flex items-center gap-1 pt-1">
                    <Flag className="w-3.5 h-3.5 shrink-0" />
                    <span>There are currently {pendingReportsCount} pending report(s) awaiting moderation.</span>
                  </p>
                )}
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-black bg-amber-400 hover:bg-amber-300 rounded-lg shadow-md shadow-amber-950/40 transition active:scale-95 whitespace-nowrap"
                >
                  <Lock className="w-3.5 h-3.5 shrink-0" />
                  <span>Unlock Moderator Console</span>
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="flex flex-col flex-1 overflow-hidden">
            ...
            {/* Tabs */}
            <div className="flex items-center justify-between px-5 bg-[#0a0f1d] border-b border-slate-800 text-xs font-medium">
              <div className="flex items-center space-x-2">
                <button
                  onClick={() => setActiveTab('reports')}
                  className={`py-3 px-3 border-b-2 font-semibold transition inline-flex items-center gap-1.5 ${
                    activeTab === 'reports'
                ...
                >
                  ...
                  <span>Flagged Packages Queue</span>
                  {pendingReportsCount > 0 && (
            ...
            <div className="p-3 sm:p-4 bg-[#0a101d] border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
              <span>Press <kbd className="px-1.5 py-0.5 bg-slate-800 border border-slate-700 rounded text-[10px] font-mono text-slate-300">Esc</kbd> or click Done</span>
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2 text-xs font-medium text-slate-200 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-lg transition"
              >
                Done
              </button>
            </div>
