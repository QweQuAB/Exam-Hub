...
export const PackageCard: React.FC<PackageCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <div>
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-1.5 flex-wrap">
            {pkg.courseCode && (
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-xs font-mono font-bold bg-cyan-950/80 text-cyan-300 border border-cyan-800/60 whitespace-nowrap">
                <GraduationCap className="w-3 h-3 text-cyan-400 shrink-0" />
                <span>{pkg.courseCode}</span>
              </span>
            )}
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-xs font-medium bg-slate-800/80 text-slate-300 border border-slate-700/60 whitespace-nowrap">
              <CategoryIcon className="w-3 h-3 text-slate-400 shrink-0" />
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button
              ...
              className="p-1.5 text-slate-500 hover:text-rose-400 hover:bg-rose-950/40 rounded-md transition opacity-70 group-hover:opacity-100"
            ...
            {isAdmin && (
              <button
                ...
                className="p-1.5 text-rose-400 hover:text-rose-200 hover:bg-rose-950/60 rounded-md transition shrink-0"
        ...
        {/* Description snippet */}
        {pkg.description && (
          <p className="text-xs text-slate-400/90 line-clamp-2 mb-3.5 leading-relaxed">
            {pkg.description}
          </p>
        )}
        ...
        <div className="flex items-center gap-1.5 mb-4 flex-wrap">
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium bg-slate-900/90 border border-slate-800 text-slate-200 whitespace-nowrap">
            <Layers className="w-3.5 h-3.5 text-cyan-400 shrink-0" />
            <span className="whitespace-nowrap">{totalQuestions} Qs</span>
          </span>

          {mcqCount > 0 && (
            <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium bg-blue-950/40 border border-blue-900/50 text-blue-300 whitespace-nowrap">
          ...
          {essayCount > 0 && (
            <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium bg-indigo-950/40 border border-indigo-900/50 text-indigo-300 whitespace-nowrap">
        ...
        {pkg.tags && pkg.tags.length > 0 && (
          <div className="flex items-center gap-1.5 flex-wrap mb-4">
            {pkg.tags.slice(0, 3).map((tag, idx) => (
              <span
                key={idx}
                className="inline-flex items-center gap-1 text-[11px] font-medium text-slate-400 bg-slate-800/50 px-2 py-0.5 rounded border border-slate-700/40 whitespace-nowrap"
              >
      ...
      <div className="pt-3.5 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400 gap-2">
        
        {/* Author info */}
        <div className="flex flex-col truncate min-w-0 max-w-[55%]">
          <span className="text-slate-300 font-medium truncate flex items-center gap-1 whitespace-nowrap">
        ...
        <div className="flex items-center space-x-1.5 shrink-0">
          ...
          <button
            ...
            className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium transition whitespace-nowrap ${
              isLiked
                ...
                : 'bg-slate-800/60 text-slate-300 hover:text-rose-400 hover:bg-slate-800 border border-slate-700/60'
            }`}
          ...
          <button
            ...
            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium bg-cyan-950/60 hover:bg-cyan-900/80 text-cyan-300 border border-cyan-800/60 hover:border-cyan-600 transition whitespace-nowrap"
