...
export const PackageDetailModal: React.FC<PackageDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/85 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6 animate-fadeIn">
      <div className="relative w-full max-w-4xl bg-[#0d1424] border border-slate-700/80 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        
        {/* Header */}
        <div className="p-4 sm:p-6 border-b border-slate-800 bg-[#0a101d]/90 flex flex-col gap-4">
          
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-1.5 flex-wrap mb-2">
                {pkg.courseCode && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-xs font-mono font-bold bg-cyan-950 text-cyan-300 border border-cyan-800/80 whitespace-nowrap">
                ...
                {(() => {
                  ...
                  return (
                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-xs font-medium bg-slate-800 text-slate-300 border border-slate-700 whitespace-nowrap">
                      <CatIcon className="w-3 h-3 text-slate-400 shrink-0" />
                      <span>{pkg.category || 'General'}</span>
                    </span>
                  );
                })()}
                <span className="px-2 py-0.5 rounded text-[11px] font-mono text-slate-400 bg-slate-900 border border-slate-800 whitespace-nowrap">
                  v{pkg.schemaVersion || 1} • {pkg.formatIdentifier}
                </span>
              </div>
            ...
            {/* Top right actions: Report & Close */}
            <div className="flex items-center gap-1.5 shrink-0">
              <button
                onClick={() => onReport(pkg)}
                className="p-2 text-slate-400 hover:text-rose-400 bg-slate-800/80 hover:bg-rose-950/40 rounded-lg transition border border-slate-700/60"
              ...
              <button
                onClick={onClose}
                className="p-2 text-slate-400 hover:text-white bg-slate-800/80 hover:bg-slate-700 rounded-lg transition shrink-0"
                    ...
                    {mcqs.map((mcq, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Explanation and Reference (when revealed) */}
                          {isRevealed && (
